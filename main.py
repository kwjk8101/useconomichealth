...
def theme(fig, h=360, title=None):
    upd = dict(
        ...
        margin=dict(l=14,r=18,t=48 if title else 32,b=40),
        hoverlabel=dict(bgcolor="#0f1e35",bordercolor="#2a4060",
                        font=dict(family=MONO,size=13,color="#dde8f5")),
    )
    if title:
        upd["title"] = dict(text=title,font=dict(size=15,color="#e8f4ff",family=MONO),x=0.01)
    fig.update_layout(**upd)
    fig.update_xaxes(gridcolor=GRID,zeroline=False,tickfont=dict(size=13,color="#b8d4f0",family=MONO),
                     linecolor="#2a4060",linewidth=1.5,showspikes=True,spikecolor="#3a5070",spikethickness=1)
    fig.update_yaxes(gridcolor=GRID,zeroline=False,tickfont=dict(size=13,color="#b8d4f0",family=MONO),
...
def make_live_chart(name, cfg, period, interval, chart_type):
    ...
    Volume is rendered in a clearly separated subplot (25% height) with muted colours
    so it never obscures the price candles.
    """
    ...
    if has_volume:
        # Two-row subplot: 78% price / 22% volume — clearly separated
        fig = make_subplots(
            rows=2, cols=1,
            shared_xaxes=True,
    ...
    if chart_type == "Candlestick" and has_ohlc:
        candle = go.Candlestick(
            ...
            name=name,
            increasing=dict(line=dict(color="#4ade80", width=1.5),
                            fillcolor="rgba(74,222,128,.75)"),
            decreasing=dict(line=dict(color="#f87171", width=1.5),
                            fillcolor="rgba(248,113,113,.75)"),
        )
        if has_volume:
        ...
        line_tr = go.Scatter(
            ...
            fill="tozeroy",
            fillcolor=f"rgba(34,211,238,0.06)",
        )
        if has_volume:
            fig.add_trace(line_tr, row=price_row, col=1)
        else:
            fig.add_trace(line_tr)

    # ── Volume trace (subplot row 2) ─────────────────────────────────────────
    if has_volume:
        vols    = hist["Volume"]
        # Very muted colours — clearly secondary to price candles
        vcolors = [
            "rgba(74,222,128,.30)" if c >= o else "rgba(248,113,113,.30)"
    ...
    layout_upd = dict(
        ...
        legend=dict(bgcolor="rgba(15,30,53,.95)", bordercolor="#1f3354",
                    borderwidth=1, font=dict(size=12, color="#c5d8f0")),
        margin=dict(l=14, r=18, t=48, b=12),
        hoverlabel=dict(bgcolor="#0f1e35", bordercolor="#2a4060",
                        font=dict(family=MONO, size=13, color="#dde8f5")),
        title=dict(text=f"{name} — {period} · {interval}",
                   font=dict(size=15, color="#e8f4ff", family=MONO), x=0.01),
        xaxis_rangeslider_visible=False,
    )
    fig.update_layout(**layout_upd)

    # Price axis (row 1)
    ax_style = dict(gridcolor=GRID, zeroline=False,
                    linecolor="#2a4060", linewidth=1.5,
                    showspikes=True, spikecolor="#3a5070", spikethickness=1)
    tick_style = dict(tickfont=dict(size=13, color="#b8d4f0", family=MONO))

    if has_volume:
        fig.update_xaxes(ax_style, **tick_style)
        fig.update_yaxes(ax_style, **tick_style)
        # Volume axis — smaller font and dimmer to signal secondary importance
        fig.update_yaxes(
            row=vol_row, col=1,
            tickfont=dict(size=10, color="#4a7090", family=MONO),
            gridcolor="rgba(31,51,84,0.4)",
            zeroline=False,
            linecolor="#1f3354",
            title_text="Volume",
...
with tab5:
    ...
    with col_x:
        # VIX — full live chart via yfinance (daily, comprehensive history)
        st.markdown('<div class="chart-section-title">VIX — Live Chart</div>',unsafe_allow_html=True)
        vix_hist = load_live_chart("^VIX", period="2y", interval="1d")
        if vix_hist is not None and not vix_hist.empty:
            fv = go.Figure()
            # Colour each day by severity zone
            ...
            fv.add_trace(go.Scatter(
                x=vix_hist.index, y=vix_vals,
                name="VIX",
                mode="lines",
                line=dict(color=C["orange"], width=2.5),
                fill="tozeroy",
                fillcolor="rgba(251,146,60,.10)",
            ))
            # Zone reference lines
            fv.add_hline(y=15, line_dash="dot",  line_color="rgba(74,222,128,.45)",  line_width=1,
                         annotation_text="15 low vol",      annotation_font_size=11, annotation_font_color="#4ade80")
            fv.add_hline(y=20, line_dash="dash", line_color="rgba(148,163,184,.55)", line_width=1.5,
                         annotation_text="20 elevated",     annotation_font_size=11, annotation_font_color="#b8d4f0")
            fv.add_hline(y=30, line_dash="dash", line_color="rgba(248,113,113,.65)", line_width=1.5,
                         annotation_text="30 crisis",       annotation_font_size=11, annotation_font_color="#f87171")
            # Shade crisis zone
            fv.add_hrect(y0=30, y1=max(float(vix_vals.max())+5, 40),
                         fillcolor="rgba(248,113,113,.05)", line_width=0)
            theme(fv, h=340, title="VIX — CBOE Volatility Index (Live Daily)")
            fv.update_layout(xaxis_rangeslider_visible=False)
            st.plotly_chart(fv, use_container_width=True)
            st.caption("Live daily VIX from yfinance (^VIX) · 2-year window · zones: <15 calm · 20 elevated · >30 crisis")
        elif "vix" in dfc.columns:
            # Fallback to FRED monthly if yfinance unavailable
            s=dfc["vix"].dropna()
            if not s.empty:
                f=go.Figure()
                ...
                f.add_hline(y=20,line_dash="dash",line_color="rgba(148,163,184,.3)",annotation_text="20 elevated",annotation_font_size=11)
                f.add_hline(y=30,line_dash="dash",line_color="rgba(248,113,113,.5)",annotation_text="30 crisis",annotation_font_size=11)
                theme(f,h=340,title="VIX — CBOE Volatility Index (FRED monthly)")
                st.plotly_chart(f,use_container_width=True)
        # CFNAI
