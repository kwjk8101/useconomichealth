"""
MACRO/SIGNAL Dashboard v14.0
============================================================
Install: pip install streamlit pandas plotly fredapi yfinance pytz requests
Run:     streamlit run macro_dashboard.py
Secrets:  FRED_API_KEY = "your_fred_key"
          FINNHUB_API_KEY = "your_finnhub_key"

v14.0 additions (Singapore-first institutional upgrade):
- NEW PAGE 6 — 🇸🇬 Singapore Hub: live STI, SGX blue-chips, S-REITs tracker,
  SGD pairs, Singapore banks (DBS/OCBC/UOB), MAS policy context,
  Market Session Clock showing open/closed status of all major exchanges in SGT
- NEW: Market Session Clock — live open/closed for SG/Tokyo/London/NY in SGT
- NEW: Crypto Dashboard — BTC/ETH/SOL/BNB prices, BTC dominance proxy
- NEW: DXY Strength Monitor — 6M chart with regime bands + asset class impact
- NEW: Global PMI Dashboard — US/EU/China/Japan Mfg + Services PMI, leading indicator
- NEW: Volatility Risk Premium — implied (VIX) vs realised vol gap
- NEW: Smart Money Flow proxy — SPY vs RSP (equal-weight) ratio as breadth signal
- ENHANCED correlations — STI, SGD, REITs added to matrix
============================================================
"""
...
SGX_BLUE_CHIPS = {
    ...
    "CapitaLand":   ("9CI.SI",  "#fb923c"),
    "Wilmar":       ("F34.SI",  "#86efac"),
    "Jardine C&C":  ("C07.SI",  "#f472b6"),
    "ST Engineering":("S63.SI", "#2dd4bf"),
    "Keppel":       ("BN4.SI",  "#94a3b8"),
}
SGX_REITS = {
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if insider_sent:
        ...
        st.info("Insider data loading. Refresh to retry.", icon="👔")
    ...
    if curve_today:
        ...
        st.warning("Yield curve data loading from FRED. Refresh to retry.", icon="📐")
    ...
    if commodities:
        ...
        st.warning("Commodity data loading. Refresh to retry.", icon="🛢️")
    ...
    with op3:
        ...
        if breadth:
            ...
            st.info("Breadth loading (samples 50 stocks). Refresh to retry.", icon="📊")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if econ_cal:
        ...
        st.warning("Calendar loading. Finnhub is primary, FRED is fallback. Refresh to retry.", icon="📅")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    page_header("🇸🇬 Singapore Hub",
                "Session clock · STI · SGX banks · S-REITs · SGD · DXY · PMI · Crypto · Smart Money · VRP")
    ...
    for i, (market, info) in enumerate(session_status.items()):
        with sess_cols[i]:
            ...
            pulse   = (f'<span style="display:inline-block;width:7px;height:7px;border-radius:50%;'
    ...
    with sti_col_c2:
        ...
        try:
            sti_h = yf.Ticker("^STI").history(period="1y")
            if not sti_h.empty:
                c_sti = sti_h["Close"].dropna()
                c_sti.index = pd.to_datetime(c_sti.index)
                if c_sti.index.tz is not None: c_sti.index = c_sti.index.tz_localize(None)
                ...
                st.markdown(
                    f'<div style="background:#0c1a2e;border:1px solid #162236;border-radius:8px;padding:14px;border-top:2px solid #22d3ee">'
                    ...
                    f'</div>', unsafe_allow_html=True)
                fig_sti=go.Figure()
                fig_sti.add_trace(go.Scatter(x=c_sti.index,y=c_sti.values,name="STI",
                    line=dict(color="#22d3ee",width=2),fill="tozeroy",fillcolor="rgba(34,211,238,.05)"))
                theme(fig_sti,h=190,fullscreen_bg=True)
                fig_sti.update_layout(margin=dict(l=8,r=8,t=8,b=16),showlegend=False)
    ...
    with mas_col2:
        ...
        mas_h2 = '<div style="background:#0c1a2e;border:1px solid #162236;border-radius:8px;padding:12px">'
        for title, val, note in sg_items:
            mas_h2 += (f'<div style="padding:7px 0;border-bottom:1px solid rgba(22,34,54,.7)">'
                       f'<div style="display:flex;justify-content:space-between"><span style="color:#dde8f5;font-size:.77rem;font-weight:600">{title}</span>'
                       f'<span style="color:#22d3ee;font-family:{MONO};font-size:.7rem;font-weight:700">{val}</span></div>'
                       f'<div style="color:#4a6a8a;font-size:.63rem;margin-top:1px">{note}</div></div>')
        mas_h2 += '</div>'
    ...
    desc("The three local banks make up ~50% of the STI. They are rate-sensitive — "
         ...
         "DBS is most international; OCBC has insurance income via Great Eastern; UOB leads ASEAN.")
        ...
        if bank_d:
            ...
            for i,(name,d) in enumerate(bank_d.items()):
                with bc[i]:
                    ...
                    st.markdown(
                        f'<div style="background:#0c1a2e;border:1px solid #162236;border-radius:8px;padding:13px;border-top:2px solid {d["color"]}">'
                        f'<div style="font-family:{MONO};font-size:.6rem;font-weight:700;color:#6a90b0;text-transform:uppercase;letter-spacing:.1em;margin-bottom:5px">{name}</div>'
                    ...
                    if d.get("hist") is not None and len(d["hist"])>5:
                        fg=go.Figure()
                        fg.add_trace(go.Scatter(x=d["hist"].index,y=d["hist"].values,
                            line=dict(color=d["color"],width=1.8),fill="tozeroy",
                            fillcolor=f"rgba({','.join(str(int(d['color'].lstrip('#')[j:j+2],16)) for j in (0,2,4))},.07)"))
                        theme(fg,h=130,fullscreen_bg=True)
                        fg.update_layout(margin=dict(l=6,r=6,t=6,b=12),showlegend=False)
    ...
    desc("S-REITs are the most popular income asset class for Singapore retail investors. "
         ...
         "MAS easing is directly bullish for S-REITs.")
    ...
    if sgx_data:
        ...
        if reit_d:
            rh=(f"<table style='width:100%;border-collapse:collapse;font-family:{MONO}'>"
                "<thead><tr style='border-bottom:1px solid #162236'>"
                + "".join(f"<th style='padding:6px 6px;color:#6a90b0;font-size:.6rem;text-transform:uppercase;text-align:{a}'>{h}</th>"
            ...
            for name,d in reit_d.items():
                ...
                rh+=(f"<tr style='border-bottom:1px solid rgba(22,34,54,.6)'>"
                     f"<td style='padding:7px 6px'><span style='color:#dde8f5;font-size:.79rem;font-weight:600'>{name}</span>"
                     f"<span style='color:#4a6a8a;font-size:.63rem;margin-left:5px'>{d['ticker']}</span></td>"
                     f"<td style='padding:7px 6px;text-align:right;color:#22d3ee;font-family:{MONO};font-size:.79rem;font-weight:700'>S${d['price']:.3f}</td>"
                     f"<td style='padding:7px 6px;text-align:right;font-family:{MONO};font-size:.73rem;font-weight:700;color:{pcc}'>{'+'if pc>=0 else''}{pc:.2f}%</td>"
                     f"<td style='padding:7px 6px;text-align:right;font-family:{MONO};font-size:.73rem;font-weight:700;color:{m1c}'>{'+'if d['pct_1m']>=0 else''}{d['pct_1m']:.2f}%</td>"
                     f"<td style='padding:7px 6px;text-align:right;font-family:{MONO};font-size:.73rem;font-weight:700;color:{ytdc}'>{'+'if d['pct_ytd']>=0 else''}{d['pct_ytd']:.2f}%</td></tr>")
            rh+="</tbody></table>"
    ...
    if dxy_data:
        ...
        with dx1:
            ...
            dxy_r=("Strong USD — Headwind for EM/Asia" if dxy_data["price"]>105
                   ...
                   else "Weak USD — Tailwind for EM/gold/crypto")
            st.markdown(
                f'<div style="background:#0c1a2e;border:1px solid #162236;border-radius:8px;padding:16px;border-top:2px solid {dxy_c}">'
                f'<div style="font-family:{MONO};font-size:.62rem;color:#6a90b0;text-transform:uppercase;letter-spacing:.1em;margin-bottom:7px">DXY Index</div>'
                ...
                f'<span style="color:{"#4ade80" if dxy_data["pct_1d"]>=0 else "#f87171"};font-weight:700">1D {"+"if dxy_data["pct_1d"]>=0 else""}{dxy_data["pct_1d"]:.2f}%</span>'
                f'<span style="color:{"#4ade80" if dxy_data["pct_1m"]>=0 else "#f87171"}">1M {"+"if dxy_data["pct_1m"]>=0 else""}{dxy_data["pct_1m"]:.2f}%</span>'
                f'<span style="color:{"#4ade80" if dxy_data["pct_3m"]>=0 else "#f87171"}">3M {"+"if dxy_data["pct_3m"]>=0 else""}{dxy_data["pct_3m"]:.2f}%</span></div>'
                f'<div style="background:#080f1c;border-radius:3px;height:5px;margin:8px 0">'
                f'<div style="width:{dxy_data["pct_pos"]:.0f}%;height:5px;border-radius:3px;background:linear-gradient(90deg,#4ade80,#f59e0b,#f87171)"></div></div>'
                f'<div style="display:flex;justify-content:space-between;font-family:{MONO};font-size:.58rem;color:#4a6a8a;margin-bottom:8px">'
                f'<span>52W Lo {dxy_data["lo52"]:.2f}</span><span style="color:{dxy_c}">{dxy_data["pct_pos"]:.0f}%ile</span><span>52W Hi {dxy_data["hi52"]:.2f}</span></div>'
                ...
                f'Gold: {"↓ Headwind" if dxy_data["price"]>102 else "↑ Tailwind"} · '
                f'EM Eq: {"↓ Headwind" if dxy_data["price"]>102 else "↑ Tailwind"} · '
                f'STI: {"↓ Moderate headwind" if dxy_data["price"]>104 else "→ Neutral"} · '
                f'BTC: {"↓ Negative" if dxy_data["price"]>103 else "↑ Positive"}</div>'
                f'</div>', unsafe_allow_html=True)
        with dx2:
            if dxy_data.get("hist") is not None and len(dxy_data["hist"])>20:
                fig_dxy=go.Figure()
                h_dxy=dxy_data["hist"].tail(252)  # 1 year of trading days
                fig_dxy.add_hrect(y0=105,y1=max(float(h_dxy.max())+2,112),fillcolor="rgba(248,113,113,.06)",line_width=0)
                fig_dxy.add_hrect(y0=min(float(h_dxy.min())-2,88),y1=97,fillcolor="rgba(74,222,128,.06)",line_width=0)
                fig_dxy.add_trace(go.Scatter(x=h_dxy.index,y=h_dxy.values,name="DXY",
                    line=dict(color="#22d3ee",width=2.5),fill="tozeroy",fillcolor="rgba(34,211,238,.04)"))
                for yl,lab,dc2 in [(105,"105 Strong USD","#f87171"),(100,"100 Neutral","#94a3b8"),(97,"97 Weak USD","#4ade80")]:
                    fig_dxy.add_hline(y=yl,line_dash="dash",
                        line_color=f"rgba({','.join(str(int(dc2.lstrip('#')[j:j+2],16)) for j in (0,2,4))},.45)",
                        line_width=1.5,annotation_text=lab,annotation_font_size=10,annotation_font_color=dc2)
                theme(fig_dxy,h=340,title="DXY — US Dollar Index (1 Year)",fullscreen_bg=True)
    ...
    if global_pmi:
        pmi_c = st.columns(min(len(global_pmi), 3))
        ...
        st.info("PMI data loading from FRED. Refresh to retry.", icon="📊")

    # ── Crypto Dashboard ──────────────────────────────────────────────────────
    sep("Crypto Market Dashboard")
    desc("Crypto is increasingly correlated with global risk appetite and liquidity. "
         "Bitcoin acts as a macro risk asset — it tends to sell with equities in risk-off and rally with liquidity expansions. "
         "For Singapore: MAS has progressive crypto regulations; Coinbase, Crypto.com and others are MAS-licensed.")
    ...
    if crypto_data:
        ...
        for i,(name,d) in enumerate(crypto_data.items()):
            with cc[i]:
                ...
                st.markdown(
                    f'<div style="background:#0c1a2e;border:1px solid #162236;border-radius:8px;'
                    f'padding:12px 12px;border-top:2px solid {d["color"]}">'
                    f'<div style="font-family:{MONO};font-size:.58rem;font-weight:700;color:#6a90b0;'
                    ...
                    f'</div></div>', unsafe_allow_html=True)
        # Normalised chart
        if any(d.get("hist") is not None for d in crypto_data.values()):
            fig_cr=go.Figure()
            for name,d in list(crypto_data.items())[:3]:
                if d.get("hist") is not None and len(d["hist"])>5:
                    base=float(d["hist"].iloc[0])
                    if base>0:
                        norm=d["hist"]/base*100
                        fig_cr.add_trace(go.Scatter(x=norm.index,y=norm.values,name=name,
                            line=dict(color=d["color"],width=2)))
            fig_cr.add_hline(y=100,line_dash="dash",line_color="rgba(148,163,184,.3)",line_width=1)
            theme(fig_cr,h=250,title="Crypto 30-Day Performance (Base 100)",fullscreen_bg=True)
    ...
    if smart_money:
        ...
        for i,(label,d) in enumerate(smart_money.items()):
            with sm_c[i%2]:
                ...
                sig=("Risk-On ↑" if (t1m>1 and is_rop) or (t1m<-1 and not is_rop)
                     else "Risk-Off ↓" if (t1m<-1 and is_rop) or (t1m>1 and not is_rop)
                     else "Neutral →")
                sc=("#4ade80" if "On" in sig else "#f87171" if "Off" in sig else "#94a3b8")
                ...
                st.markdown(
                    ...
                    f'<span style="font-family:{MONO};font-size:.72rem;font-weight:700;color:{sc}">{sig}</span></div>'
                ...
                if d.get("hist") is not None and len(d["hist"])>5:
                    fsm=go.Figure()
                    fsm.add_trace(go.Scatter(x=d["hist"].index,y=d["hist"].values,
                        line=dict(color=sc,width=1.7),fill="tozeroy",
                        fillcolor=f"rgba({','.join(str(int(sc.lstrip('#')[j:j+2],16)) for j in (0,2,4))},.06)"))
                    theme(fsm,h=140,fullscreen_bg=True)
                    fsm.update_layout(margin=dict(l=8,r=8,t=6,b=12),showlegend=False)
                    fsm.update_xaxes(showticklabels=False)
                    st.plotly_chart(fsm,use_container_width=True)
    ...
    desc("<strong>VRP = VIX (implied vol) minus 30-day realised vol</strong>. "
         ...
         "When options are cheap (VRP < 0), market is underpricing risk — buy protection. "
         "This is a professional-grade signal used by vol arbitrage funds globally.")
    ...
    if vol_rp:
        ...
        with vrpa:
            ...
            vl=("Options Expensive" if vol_rp["vrp"]>5 else "Slight Premium — Normal" if vol_rp["vrp"]>2
                else "Fair Value" if vol_rp["vrp"]>-1 else "Options Cheap — Buy Protection")
            st.markdown(
        ...
        with vrpb:
            if vol_rp.get("vrp_hist") is not None and len(vol_rp["vrp_hist"])>5:
                ...
                theme(fig_vrp,h=280,title="Volatility Risk Premium — 60 Day History",fullscreen_bg=True)
